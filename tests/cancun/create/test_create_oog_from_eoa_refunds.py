...
def build_init_code(
    ...
    if refund_type == RefundType.SSTORE_DIRECT:
        ...
        refund_code = Op.POP(Op.CALL(Op.GAS, helpers.sstore_refund, 0, 0, 0, 0, 0))
        ...
        refund_code = Op.POP(Op.CALL(Op.GAS, helpers.selfdestruct, 0, 0, 0, 0, 0))
        ...
        refund_code = (
            Op.SSTORE(1, 1)
            + Op.SSTORE(1, 0)
            + Op.EXTCODECOPY(helpers.init_code, 0, 0, Op.EXTCODESIZE(helpers.init_code))
    ...
    if oog_scenario == OogScenario.NO_OOG:
        ...
        if refund_type in (
            ...
            ending = Op.RETURN(Op.ADD(Op.EXTCODESIZE(helpers.init_code), 1), 5000)
        else:
...
def test_create_oog_from_eoa_refunds(
    ...
    if oog_scenario == OogScenario.NO_OOG:
        ...
        if refund_type == RefundType.NESTED_CREATE:
            ...
            nested_created = compute_create_address(address=created_address, nonce=1)
    ...
    if fork.header_bal_hash_required():
        if oog_scenario == OogScenario.NO_OOG:
            ...
            expected_nonce = (
                2 if refund_type in (RefundType.NESTED_CREATE, RefundType.NESTED_CREATE2) else 1
            )
            created_bal = BalAccountExpectation(
                ...
                storage_changes=[
                    BalStorageSlot(
                        slot=0,
                        slot_changes=[BalStorageChange(block_access_index=1, post_value=1)],
                    ),
                ],
                storage_reads=(
                    # noop write 0 -> 1 -> 0
